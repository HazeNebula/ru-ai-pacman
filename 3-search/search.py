...
def constructPath( cameFrom, current ):
	...
	while (current[1] is not None):
		...
		for parent in cameFrom:
			if (parent[1][0] == current[0]):
				current = parent[0]
				break
	...
	while (not frontier.isEmpty()):
		...
		for successor in successors:
			...
			if (not isInSet):
				...
				cameFrom.append( (current, successor) )
...
def getPathCost( cameFrom, current ):
	pathSegmentCosts = []

	while (current[1] is not None):
		pathSegmentCosts.insert( 0, current[2] )
	...
	pathCost = 0
	for pathSegmentCost in pathSegmentCosts:
		pathCost = pathCost + pathSegmentCost
...
def uniformCostSearch( problem ):
	...
	while (not frontier.isEmpty()):
		...
		successors = problem.getSuccessors( current[0] )
		pathCost = getPathCost( cameFrom, current )
		for successor in successors:
			isInSet = False
			for node in visited:
				if (successor[0] == node):
			...
			if (not isInSet):
				frontier.push( successor, pathCost + successor[2] )
				cameFrom.append( (current, successor) )
...
def aStarSearch( problem, heuristic = nullHeuristic ):
	...
	while (not frontier.isEmpty()):
		...
		for successor in successors:
			...
			if (not isInSet):
				frontier.push( successor, pathCost + successor[2] + heuristic( successor[0], problem ) )
				cameFrom.append( (current, successor) )
